...
export function MonthlyAttendanceDetail({ employeeId, initialDate }) {
  ...
  const generateAttendanceHistory = () => {
    ...
    return Array.from({ length: daysInMonth }, (_, i) => {
      ...
      if (randomStatus === "early") {
        ...
        checkOutHour = 16 + Math.floor(Math.random() * 1) // Về sớm
        const earlyCheckInMinutes = (8 - checkInHour) * 60
        const earlyCheckOutMinutes = Math.floor(Math.random() * 60) + 30
        earlyMinutes = earlyCheckInMinutes + earlyCheckOutMinutes
        timeDifference = `Sớm ${Math.floor(earlyMinutes / 60)}h ${earlyMinutes % 60}p`
      } else if (randomStatus === "late") {
        checkInHour = 8 + Math.floor(Math.random() * 2)
        const lateCheckInMinutes = (checkInHour - 8) * 60 + Math.floor(Math.random() * 60)
        lateMinutes = lateCheckInMinutes
        timeDifference = `Muộn ${Math.floor(lateMinutes / 60)}h ${lateMinutes % 60}p`
      } else {
  ...
  const summary = {
    ...
    totalWorkHours: attendanceHistory
      ...
      .toFixed(1),
    totalOTHours: attendanceHistory.reduce((sum, day) => sum + (day.overtimeHours || 0), 0).toFixed(1),
    totalLateHours: (attendanceHistory.reduce((sum, day) => sum + (day.lateMinutes || 0), 0) / 60).toFixed(1),
    totalEarlyHours: (attendanceHistory.reduce((sum, day) => sum + (day.earlyMinutes || 0), 0) / 60).toFixed(1),
  }
  ...
  const prepareExcelData = () => {
    return attendanceHistory.map((record) => {
      ...
      return {
        ...
        "OT (giờ)": record.overtimeHours || 0,
        "Đi Muộn (phút)": record.lateMinutes || 0,
        "Về Sớm (phút)": record.earlyMinutes || 0,
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <ExportExcelButton
          ...
          filename={`Chi-Tiet-Cham-Cong-${employee?.name || "Nhan-Vien"}-${month}-${year}`}
          sheetName="Chi Tiết Chấm Công"
          buttonText="Xuất Excel Chi Tiết"
          highlightCondition={(row) => row._status === "late" || row._status === "absent"}
                  ...
                  <TableBody>
                    ...
                    {attendanceHistory.map((record, index) => (
                      ...
                      >
                        ...
                        <TableCell>{record.overtimeHours || 0} giờ</TableCell>
                        <TableCell>
                          {record.faceImage ? (
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <SummaryCard title="Số Ngày Đi Đúng Giờ" value={summary.ontimeDays} color="green" />
                <SummaryCard title="Số Ngày Đi Muộn" value={summary.lateDays} color="yellow" />
                <SummaryCard title="Số Ngày Đi Sớm" value={summary.earlyDays} color="blue" />
                <SummaryCard title="Số Ngày Vắng Mặt" value={summary.absentDays} color="red" />
                <SummaryCard title="Tổng Số Giờ Làm" value={`${summary.totalWorkHours} giờ`} color="purple" />
                <SummaryCard title="Tổng Số Giờ OT" value={`${summary.totalOTHours} giờ`} color="indigo" />
                <SummaryCard
                  title="Tổng Số Giờ Đi Muộn"
                  value={`${summary.totalLateHours} giờ`}
                  color="orange"
                  icon={<TrendingUp className="h-4 w-4" />}
                />
                <SummaryCard
                  title="Tổng Số Giờ Về Sớm"
                  value={`${summary.totalEarlyHours} giờ`}
                  color="cyan"
 ...
 * @param {string} props.color - Màu sắc của thẻ
 * @param {JSX.Element} props.icon - Icon hiển thị
 */
function SummaryCard({ title, value, color = "gray", icon }) {
  const colorClasses = {
